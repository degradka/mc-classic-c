...
void Connection_kick(Connection* c, const char* reason) {
    ...
    // matches PendingDisconnect: a 40 tick grace period rather than an
    // immediate close, so this Disconnect packet actually reaches the
    // client before the socket goes away. Connection_tick counts it down
    c->pendingClose = true;
    c->closeGraceTicks = 40;
...
static void handleLogin(Connection* c, const unsigned char* f) {
    ...
    c->loggedIn = true;
    Log_info("%s logged in as %s", c->remoteAddress, c->username);
