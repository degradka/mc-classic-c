...
// matches SpawnPlayer with id < 0: the server specifying where the local
...
static int init(Level* lvl, LevelRenderer* lr, Player* p) {
    ...
    // c0.0.16a_02: a host argument switches to multiplayer, keeping the boot
    // level above only as a placeholder until LevelFinalize replaces it
    if (gConnectHost[0]) {
...
static void tick(Player* p, GLFWwindow* w) {
    ...
    // the local player's position every tick, no dirty check, no rate
