...
static int init(Level* lvl, LevelRenderer* lr, Player* p) {
    ...
    if (gIsFullscreen) {
        window = glfwCreateWindow(mode->width, mode->height, "Minecraft 0.0.17a", monitor, NULL);
    } else {
        window = glfwCreateWindow(gWinWidth, gWinHeight, "Minecraft 0.0.17a", NULL, NULL);
    }
...
static void drawGui(float partialTicks) {
    ...
    Font_drawShadow(&gFont, &hudTess, "0.0.17a", 2, 2, 0xFFFFFF);
