                        ...
                        int layer, int x, int y, int z)
{
    ...
    // Bush uses atlas slot 15
    float u0, v0, u1, v1;
    u0 = (float)(self->textureId % 16) / 16.0f;
    u1 = u0 + 0.0624375f;
    v0 = (float)(self->textureId / 16) / 16.0f;
    v1 = v0 + 0.0624375f;

    Tessellator_color(t, 1.0f, 1.0f, 1.0f);

    // two crossed diagonal planes, inset from the block edges rather than
    // touching the corners, matching the real cross shaped sapling render
    const int rots = 2;
